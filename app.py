...
#         # ✅ Check if it is a room image
#         if not is_room_image(image_path):
#             logger.warning("Uploaded image is not recognized as a room.")
#             os.remove(image_path)
#             return jsonify({'error': 'Please upload a valid room image'}), 400
...
def recommend():
    if request.method == 'GET':
        return jsonify({'message': 'Use POST method with image and category to get recommendations'})
    ...
    try:
        ...
        if not is_room_image(image_path):
            os.remove(image_path)
            return jsonify({'error': 'Upload a valid room image'}), 400
